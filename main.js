...
var change = function (s) {
    ...
    var length = out1.join("").length;
    var explanation = document.getElementById("explanation");
    explanation.innerText = out1.join("") + "  # Implicit input\n";
    
    for (var x = 0; x < out1.length; x++) {
        if (out1[x] != "") {
            var before = out1.slice(0, x).join("").length;
            var after = out1.slice(x + 1).join("").length;
            explanation.innerText += " ".repeat(before) + out1[x] + " ".repeat(after) + "  # " + uppercaseFirstLetter(arr[x].replace("_", " ")) + "\n";
        }
    }
...
var copyLink = function () {
    ...
    var url = "https://Not-Thonnu.github.io/Verbose/index.html?input=" + encodeURIComponent(input);
